...
import {
    ...
    PopupRevealerProps,
    PopupWindowProps,
} from "../lib/types/popupwindow";
import { Exclusivity, GtkWidget } from "../lib/types/widget";
...
export const Padding = ({ name, opts }: PaddingProps): JSX.Element => (
    <eventbox
        ...
        canFocus={false}
        setup={(self: EventBox) =>
            self.connect("button-press-event", () => App.toggle_window(name))
        }
    >
        <box />
    </eventbox>
...
const PopupRevealer = ({
    ...
    child,
    transition,
}: PopupRevealerProps): JSX.Element => (
    <box css={"padding: 1px"}>
        <revealer
            ...
            setup={(self: Revealer) => {
                App.connect("window-toggled", (app) => {
                    self.revealChild =
                        app.get_window(name)?.is_visible() ?? false;
                });
            }}
        >
